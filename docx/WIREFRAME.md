# 와이어프레임 (Wireframe): TimeFilm

본 문서는 아스키 아트(ASCII Art) 텍스트를 기반으로 작성된 TimeFilm 클론 서비스의 핵심 두 가지 화면(퍼블릭 모바일 뷰, 관리자 대시보드 뷰) 기획안입니다.

## 1. 퍼블릭 모바일 뷰 (방문자 공유 화면)
방문자가 고유 주소(예: `timefilm.com/@username`)로 접속했을 때 렌더링되는 최종 모바일 프로필 화면입니다. 모바일 기기의 세로 비율에 최적화되어 표시됩니다.
...
```text
+-----------------------------------------------------------------------------------+
|  [TimeFilm 로고]    링크 편집    테마 디자인    통계 데이터            [프로필 관리]|
+-----------------------------------------------------------------------------------+
|                                               |                                   |
